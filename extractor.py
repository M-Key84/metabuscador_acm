...
class ExtractorInmobiliario:
    ...
    def raspar_portal_real(self, municipio, barrio, es_rph):
        """
        Intenta obtener ofertas reales desde los portales.
        Si no se consiguen suficientes (menos de 4), completa con simulación.
        """
        muestras = []
        # Intentar cada portal
        ...
        # Si no se obtuvieron al menos 4 muestras reales, completar con simulación
        if len(muestras) < 4:
            faltan = 6 - len(muestras)
            simuladas = self.raspar_portal_simulado(municipio, barrio, es_rph)[:faltan]
            muestras += simuladas
        return muestras[:8]  # máximo 8 muestras