...
# Inicialización completa de la sesión
...
def fitto_corvini(area, edad, vida_util, conservacion, costo_m2):
    """
    Calcula la depreciación según el método Fitto Corvini.
    Retorna dict con coeficiente, valor depreciado por m² y valor total construcción.
    """
    clase = {"Buena": 3, "Regular": 4, "Mala": 5}.get(conservacion, 3)
    edad_pct = (edad / vida_util) * 100
    # Coeficiente de depreciación (tabla simplificada basada en la práctica de Diego)
    if edad_pct <= 5:
        coef = 0.05
    elif edad_pct <= 10:
        coef = 0.10
...
def seleccionar_top5_por_area(muestras, area_ref, es_rph):
    """
    Devuelve las 5 muestras con el área más cercana a la del inmueble objetivo.
    """
...
# ==================== GENERACIÓN DEL EXCEL PARA RPH = SÍ ====================
def generar_excel_rph_si(datos_obj, muestras):
    ...
    headers_ficha = ["Variable Requerida", "Valor / Especificación"]
    ws1.append([]); ws1.append([]); ws1.append(headers_ficha)
    for c in [1, 2]:
    ...
    for i, (campo, val) in enumerate(variables, start=6):
    ...
    ws2.append([]); ws2.append([]); ws2.append(headers_ws2)
    ...
    for idx, m in enumerate(muestras):
        ...
        area_val = m["area_construida"]  # RPH = construida
    ...
    # Constantes en celdas ocultas para los límites del 15%
    ws3["D10"] = 1.15
    ...
    # En la ficha, el Área Construida está en B12 (ver lista de variables)
    ws3["B15"] = "='1. Ficha Inmueble Objetivo'!B12"
...
# ==================== GENERACIÓN DEL EXCEL PARA RPH = NO ====================
def generar_excel_rph_no(datos_obj, muestras_completas, top5, calc_fitto):
    ...
    # Función interna para escribir una hoja de avalúo (completa o depurada)
    def escribir_hoja_avaluo(ws, datos, muestras, calc, titulo):
        ws["A2"] = titulo
        ...
        for i, m in enumerate(muestras, 1):
            ...
            # Valor depreciado construcción = costo m2 * área
            valor_depreciado_const = calc["valor_depreciado_m2"] * area_construida_m if area_construida_m > 0 else 0
        ...
        if n > 0:
            # Estadísticas
            fila_actual = fila
            ws.cell(row=fila_actual, column=9, value="Promedio").font = font_bold
            ws.cell(row=fila_actual, column=10, value=f"=AVERAGE(J10:J{10+n-1})").number_format = "#,##0"
            fila_actual += 1
            ws.cell(row=fila_actual, column=9, value="Desviación Estándar").font = font_bold
            ws.cell(row=fila_actual, column=10, value=f"=STDEV(J10:J{10+n-1})").number_format = "#,##0"
            fila_actual += 1
            ws.cell(row=fila_actual, column=9, value="Coeficiente Variación").font = font_bold
            ws.cell(row=fila_actual, column=10, value=f"={fila_actual-1}/{fila_actual-2}").number_format = "0.00%"
            fila_actual += 1
            ws.cell(row=fila_actual, column=9, value="Coeficiente Asimetria").font = font_bold
            ws.cell(row=fila_actual, column=10, value=f"=SKEW(J10:J{10+n-1})").number_format = "0.00"
            fila_actual += 1
            ws.cell(row=fila_actual, column=9, value="Límite Superior").font = font_bold
            ws.cell(row=fila_actual, column=10, value=f"={fila_actual-4}*1.15").number_format = "#,##0"
            fila_actual += 1
            ws.cell(row=fila_actual, column=9, value="Límite Inferior").font = font_bold
            ws.cell(row=fila_actual, column=10, value=f"={fila_actual-5}*0.85").number_format = "#,##0"
            fila_actual += 1
            ws.cell(row=fila_actual, column=9, value="VALOR ADOPTADO").font = font_bold
            ws.cell(row=fila_actual, column=10, value=f"={fila_actual-6}").number_format = "#,##0"
            fila_promedio = fila_actual - 6

        # Liquidación final
        fila_actual += 2
        ws.cell(row=fila_actual, column=1, value="IDENTIFICACIÓN DEL BIEN").font = font_bold
        ws.cell(row=fila_actual, column=2, value="ÁREA")
        ws.cell(row=fila_actual, column=3, value="VALOR M2")
        ws.cell(row=fila_actual, column=4, value="VALOR TOTAL")
        fila_actual += 1
        ws.cell(row=fila_actual, column=1, value="LOTE DE TERRENO")
        ws.cell(row=fila_actual, column=2, value=datos.get("area_terreno", 0))
        ws.cell(row=fila_actual, column=3, value=f"={fila_promedio+4}")
        ws.cell(row=fila_actual, column=4, value=f"=B{fila_actual}*C{fila_actual}").number_format = "#,##0"
        fila_actual += 1
        ws.cell(row=fila_actual, column=1, value="CONSTRUCCIÓN")
        ws.cell(row=fila_actual, column=2, value=datos.get("area_construida", 0))
        ws.cell(row=fila_actual, column=3, value=calc["valor_depreciado_m2"]).number_format = "#,##0"
        ws.cell(row=fila_actual, column=4, value=f"=B{fila_actual}*C{fila_actual}").number_format = "#,##0"
        fila_actual += 1
        ws.cell(row=fila_actual, column=1, value="VALOR TOTAL DEL INMUEBLE").font = font_bold
        ws.cell(row=fila_actual, column=4, value=f"=SUM(D{fila_actual-2}:D{fila_actual-1})").number_format = "#,##0"

    # ---------- Hoja 1: AVALUO CASA PRIMER PISO (8 muestras) ----------
    ws1 = wb.active
    ws1.title = "AVALUO CASA PRIMER PISO"
    escribir_hoja_avaluo(ws1, datos_obj, muestras_completas, calc_fitto, "AVALUO CASA PRIMER PISO")

    # ---------- Hoja 2: AVALUO DEPURADO (5 mejores) ----------
    ws2 = wb.create_sheet(title="AVALUO DEPURADO")
    escribir_hoja_avaluo(ws2, datos_obj, top5, calc_fitto, "AVALUO CASA PRIMER PISO")

    # ---------- Hoja 3: FITTO CORVINI ----------
    ws3 = wb.create_sheet(title="FITTO CORVINI")
    ws3["A2"] = "UNIFAMILIAR MEDIO - COSTO DE REPOSICIÓN"
    ws3["A3"] = "Calculo Valor Depreciado (VIVIENDA DE USO HABITACIONAL - SUJETO)."
    ws3["A4"] = "Método Fitto Corvini - Artículo 37 Numeral 9 - Depreciación (Resolución 620 de 2008)."
    headers_fitto = ["DESCRIPCIÓN", "VALOR", "UNIDAD / OBSERVACIÓN"]
    for c, h in enumerate(headers_fitto, 1):
        cell = ws3.cell(row=6, column=c, value=h)
        cell.font = font_header
        cell.fill = fill_header
        cell.alignment = Alignment(horizontal="center")

    data_fitto = [
        ("Construcción aproximada", datos_obj.get("area_construida", 0), "M2"),
        ("Edad", datos_obj.get("edad_const", 0), "años aproximadamente"),
        ("Conservación", "", datos_obj.get("conservacion", "")),
        ("Metodología (Depreciación acumulada)", "", "Fitto Corvini"),
        ("Costo directo de construcción actual por unidad", f"${datos_obj.get('costo_reposicion_m2',0):,.0f}", "Pesos por M2"),
        ("Vida técnica", 70, "años"),
        ("Depreciación Calificación", 3.5, "Clase"),
        ("Coeficiente de depreciación", calc_fitto["coef"], ""),
        ("Valor a depreciar", f"${datos_obj.get('costo_reposicion_m2',0) - calc_fitto['valor_depreciado_m2']:,.0f}", "Pesos"),
        ("Valor depreciado de la construcción M2", f"${calc_fitto['valor_depreciado_m2']:,.0f}", "Pesos"),
        ("Valor M2 de la construcción", f"${calc_fitto['valor_depreciado_m2']:,.0f}", "Pesos por M2"),
        ("VALOR TOTAL CONSTRUCCIÓN", f"${calc_fitto['valor_total_construccion']:,.0f}", "Pesos"),
    ]
    for r, (desc, val, unit) in enumerate(data_fitto, 7):
        ws3.cell(row=r, column=1, value=desc).font = font_regular
        ws3.cell(row=r, column=2, value=val).font = font_bold
        ws3.cell(row=r, column=3, value=unit).font = font_regular
        for c in range(1, 4):
...
if st.button("🚀 Consultar Metabuscador y Procesar Homologación", use_container_width=True):
    ...
    st.session_state.media, st.session_state.desv, st.session_state.cv, st.session_state.asimetria, \
    st.session_state.lim_sup, st.session_state.lim_inf, st.session_state.aprobado = motor.analizar_estadistica_igac(st.session_state.valores_m2)

    st.session_state.top5_muestras = seleccionar_top5_por_area(st.session_state.muestras, area_referencia, rph)

    if rph == "NO":
        st.session_state.calc_fitto = fitto_corvini(area_construida, edad_const, 70, conservacion, costo_reposicion_m2)
    else:
        st.session_state.calc_fitto = {}
...
if st.session_state.procesado:
    ...
    if st.session_state.datos_inmueble["rph"] == "NO":
        excel_final = generar_excel_rph_no(
            st.session_state.datos_inmueble,
            st.session_state.muestras,
            st.session_state.top5_muestras,
            st.session_state.calc_fitto
        ...
        excel_final = generar_excel_rph_si(
            st.session_state.datos_inmueble,
            st.session_state.top5_muestras
        )
